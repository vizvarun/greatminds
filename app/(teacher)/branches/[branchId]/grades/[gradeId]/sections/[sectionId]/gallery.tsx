...
import GalleryDeleteAlert from "./GalleryDeleteAlert";
...
export default function GalleryScreen() {
  ...
  // Add state for preview group and index
  const [previewGroupImages, setPreviewGroupImages] = useState<string[]>([]);
  const [previewIndex, setPreviewIndex] = useState<number>(0);

  // Add PanResponder for swipe gestures in preview modal
  ...
  // Reset preview index when modal is closed or opened for a new image
  useEffect(() => {
    if (!previewVisible) {
      setPreviewIndex(0);
  ...
  // Delete group handler (show modal)
  const handleDeleteGroup = (group: GalleryGroup) => {
  ...
  const confirmDeleteGroup = async () => {
    ...
    setDeleting(true);
    try {
      await deleteGalleryGroup(deleteTargetGroup.groupid);
      loadGroups();
      setCustomAlertTitle("Success");
      setCustomAlertMessage("Gallery group successfully deleted.");
  ...
  const renderGroup = ({ item }: { item: GalleryGroup }) => (
    <View style={styles.entryCard}>
      <View style={styles.groupImagesWrapper}>
        ...
        {/* Delete icon at top right, no background */}
        <TouchableOpacity
  ...
  // Add state for image loading in modal
  ...
  const pickImage = async (mode: "camera" | "gallery") => {
    ...
    // Show modal instantly after selection (even before images are loaded)
    let uris: string[] = [];
    if (!result.canceled) {
      uris =
        "assets" in result
      ...
      setImageLoadingStates(Object.fromEntries(uris.map((uri) => [uri, true])));
      setMediaModalVisible(true); // <-- ensure modal opens instantly after selection
    }
  };

  // Add state for keyboard visibility in modal
  const [captionKeyboardVisible, setCaptionKeyboardVisible] = useState(false);
  const captionInputAccessoryViewID = "galleryCaptionInput";
  const captionInputRef = useRef<TextInput>(null);

  // Clear caption when media modal is opened
  useEffect(() => {
  ...
  // Reset image loading states when modal is closed or opened for new images
  useEffect(() => {
    if (!mediaModalVisible) {
  ...
  useEffect(() => {
    if (!mediaModalVisible) return;
    const showListener = Keyboard.addListener("keyboardDidShow", () =>
      setCaptionKeyboardVisible(true)
    );
    const hideListener = Keyboard.addListener("keyboardDidHide", () =>
      setCaptionKeyboardVisible(false)
    );
    return () => {
      showListener.remove();
      hideListener.remove();
  ...
  const handleUpload = async () => {
    if (!sectionId || !userProfile?.user?.id || selectedImages.length === 0)
      return;
    setUploading(true);
    try {
      ...
      await api.post(
        `/upload/files/section?section_id=${sectionId}&user_id=${
          userProfile.user.id
      ...
      // Only close modal after upload is done
      setMediaModalVisible(false);
      setSelectedImages([]);
      setCaption("");
      loadGroups();
    } catch (e) {
      setCustomAlertTitle("Upload Failed");
      setCustomAlertMessage("Could not upload media. Try again.");
      setCustomAlertType("error");
      setCustomAlertVisible(true);
    }
    setUploading(false);
  ...
  return (
    ...
    >
      <View style={styles.container}>
          ...
          >
            ...
            >
              ...
              >
                ...
                />
                {/* Keyboard dismiss bar for iOS */}
                {Platform.OS === "ios" && (
                  <KeyboardDismissBar nativeID={captionInputAccessoryViewID} />
                )}
                {/* Keyboard dismiss bar for Android */}
                {/* {Platform.OS === "android" && captionKeyboardVisible && (
                  <View
                    style={{
        ...
        />
        {/* Custom Alert for download/upload errors */}
        <CustomAlert
...
const styles = StyleSheet.create({
  ...
  groupImagesWrapper: {
    ...
    minHeight: 70,
    marginBottom: 8,
    // width to fit 4 images + margins
    width: 4 * 70 + 3 * 8 + 16, // 4 images, 3 gaps, 8px padding each side
    maxWidth: "100%",
  ...
  groupImagesRow: {
    ...
    alignItems: "center",
    padding: 8,
    paddingRight: 40, // add space for delete/more overlays
    backgroundColor: "transparent",
    zIndex: 1,
  },
  groupImage: {
